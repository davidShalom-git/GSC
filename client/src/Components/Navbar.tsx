...
const Navbar = () => {
    ...
    return (
        <div ref={ref} className="h-88 md:h-64 overflow-y-scroll [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            <nav className={`fixed top-0 left-0 w-full flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 ${isScrolled ? "bg-white shadow-md text-gray-700 py-3 md:py-4" : "bg-white/10 backdrop-blur-md border-b border-white/20 text-white py-4 md:py-6"}`}>
                ...
                <div className="hidden md:flex items-center gap-4 lg:gap-8">
                    {navLinks.map((link, i) => (
                        <a key={i} href={link.path} className={`group flex flex-col gap-0.5 ${isScrolled ? "text-black" : "text-black"}`}>
                            {link.name}
